package com.K1ll4a.global.facility;
...
import com.K1ll4a.global.tools.Validatable;
...
public class Mclass implements Comparable<Mclass>,Serializable, Validatable {
    ...
    @JsonCreator
    public Mclass(@JsonProperty("id") long id,@JsonProperty("name") String name, @JsonProperty("coordinates") Coordinates coordinates,@JsonProperty("creationDate") String creationDate, @JsonProperty("price") Double price,@JsonProperty("manufactureCost") float manufactureCost, @JsonProperty("unitOfMeasure") UnitOfMeasure unitOfMeasure, @JsonProperty("manufacturer") Organization manufacturer,@JsonProperty("user_id") int user_id,@JsonProperty("login") String login) {
        this.id = (id>0) ? id : UUID.randomUUID().getMostSignificantBits() & Long.MAX_VALUE;
    ...
    public Mclass(@JsonProperty("name") String name, @JsonProperty("price") Double price, @JsonProperty("coordinates") Coordinates coordinates, @JsonProperty("manufactureCost") Float manufactureCost, @JsonProperty("unitOfMeasure") UnitOfMeasure unitOfMeasure, @JsonProperty("manufacturer") Organization manufacturer, @JsonProperty("user_id") int user_id, @JsonProperty("login") String login) {
        ...
        this.id = ThreadLocalRandom.current().nextLong(1, 1001);
        long randomEpochSecond = ThreadLocalRandom.current().nextLong(0, LocalDateTime.now().toEpochSecond(ZoneOffset.UTC));
        this.creationDate = LocalDateTime.ofEpochSecond(randomEpochSecond, 0, ZoneOffset.UTC);
        this.creationDate = creationDate.atZone(ZoneOffset.UTC).toLocalDateTime();
    }
    ...
    private long generateRandomId() {
        Random random = new Random();
        // Генерируем случайный ID, который больше 0
        return random.longs(1, Long.MAX_VALUE).findFirst().getAsLong();
    }
